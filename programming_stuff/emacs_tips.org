...
** ASCII Art
+ Install ditaa
  + MacOS: ~brew install ditaa~
  + Linux: ~sudo dnf install ditaa~
+ Enable ditaa:
  #+BEGIN_SRC elisp
...
+ Example:
  ...
  #+END_SRC
* Todoist
+ To enable todoist integration with emacs, install the ~todoist~ package
+ Setup
  + ~(setq todoist-token "<token value>")~
+ To run: ~M-x todoist~
